给你一个有 `n` 个节点的 **有向带权** 图，节点编号为 `0` 到 `n - 1` 。图中的初始边用数组 `edges` 表示，其中 `edges[i] = [from<sub>i</sub>, to<sub>i</sub>, edgeCost<sub>i</sub>]` 表示从 `from<sub>i</sub>` 到 `to<sub>i</sub>` 有一条代价为 `edgeCost<sub>i</sub>` 的边。
